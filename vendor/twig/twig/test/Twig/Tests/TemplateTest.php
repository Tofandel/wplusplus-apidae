...
class Twig_Tests_TemplateTest extends \PHPUnit\Framework\TestCase {
	...
	public function getAttributeExceptions() {
		return array(
			...
			array(
				'{{ array_access.a }}',
				'Neither the property "a" nor one of the methods "a()", "geta()"/"isa()"/"hasa()" or "__call()" exist and have public access in class "Twig_TemplateArrayAccessObject" in "%s" at line 1.'
			),
			array(
				'{% from _self import foo %}{% macro foo(obj) %}{{ obj.missing_method() }}{% endmacro %}{{ foo(array_access) }}',
				'Neither the property "missing_method" nor one of the methods "missing_method()", "getmissing_method()"/"ismissing_method()"/"hasmissing_method()" or "__call()" exist and have public access in class "Twig_TemplateArrayAccessObject" in "%s" at line 1.'
			),
			array(
	...
	public function testDisplayBlockWithUndefinedParentBlock() {
		...
		$template->displayBlock( 'foo', array(), array(
			'foo' => array(
				new Twig_TemplateTest( $twig, 'index.twig' ),
				'block_foo'
			)
		), false );
	}
